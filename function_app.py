...
def tweet(product, status):
    try:
        ...
        logging.info(json.dumps(response.data, indent=4, sort_keys=True))
    ...
    try:
        ...
        for product in data['products']:
            ...
            if title in new_products:
                logging.info(f"New product added: {title}")
                tweet(product, "NEW PRODUCT")
            elif title in restocked_products:
                logging.info(f"Product back in stock: {title}")
                tweet(product, "BACK IN STOCK")
            elif title in out_of_stock_products:
                logging.info(f"Product out of stock: {title}")
